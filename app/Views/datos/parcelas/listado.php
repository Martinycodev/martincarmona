...
<div class="search-container">
    <div class="search-box">
        <input type="text" id="searchInput" class="search-input" placeholder="🔍 Buscar parcelas (escribe 3+ caracteres para filtrar)..." autocomplete="off">
            ...
            <?php else: ?>
                <?php foreach ($parcelas as $parcela): ?>
                    <tr data-id="<?= htmlspecialchars($parcela['id']) ?>">
                        ...
                        <td class="actions">
                            <a href="<?= $this->url('/datos/parcelas?id=' . $parcela['id']) ?>" class="btn-icon btn-view" title="Ver detalles">
                                👁️
                            </a>
                            <a href="<?= $this->url('/parcelas/editar/' . $parcela['id']) ?>" class="btn-icon btn-edit" title="Editar">
                                ✏️
                            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tableSearch = initializeTableSearch({
        ...
        searchFields: ['nombre', 'ubicacion', 'dueño'],
        minSearchLength: 3,
